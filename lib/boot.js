...
if (!token && config.is_app_engine) {
  window.location = 'https://web.simplenote.com/signin';
}

if (cookie.token && cookie.email) {
  // Set account name from 'email' token
  store.dispatch(setAccountName(decodeURIComponent(cookie.email)));
}
...
let props = {
  ...
  onSignOut: () => {
    ...
    if (config.is_app_engine) {
      // Clear token and email cookies upon signout
