...
const getTitleAndPreview = content => {
  // grab the first N=10 characters of the first line
  const titlePattern = new RegExp( `\s*([^\n]{1,${ maxTitleChars }})`, 'g' );
  const titleMatch = titlePattern.exec( content );
  if ( ! titleMatch ) {
    return defaults; // do something appropriate here
  }
  const [ , title ] = titleMatch;

  // skip leading whitespace, then grab first N=10 characters of body
  const previewPattern = new RegExp( `[^\n]*\n+[ \t]*([^]{0,${ maxPreviewChars }})`, 'g' );

  return { title, preview: matchUntilLimit( previewPattern, content ) };
}
...
export const noteTitleAndPreview = note => {
  ...
  const preview = getTitleAndPreview( content )
  return preview;
};
